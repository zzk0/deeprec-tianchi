...
# Modified to control DeepRec train option 1111
...
class _ActionsContainer(object):
    ...
    def add_argument(self, *args, **kwargs):
        ...
        # NOTE(zzk0): set bf16 here
        # if action.dest == 'bf16':
        #     action.default = True

        if action.dest == 'smartstaged':
            action.default = True
...
def get_argparse_path():
  ...
  for package in packages:
    candidate = os.path.join(package, filename)
    if os.path.exists(candidate):
      return candidate
    path = Path(package)
    candidate = os.path.join(path.parent.absolute(), filename)
    if os.path.exists(candidate):
      return candidate


def modify_argparse():
  argparse_path = get_argparse_path()
  fout = open(argparse_path, "w")
  fout.write(argparse_text)
  fout.close()
